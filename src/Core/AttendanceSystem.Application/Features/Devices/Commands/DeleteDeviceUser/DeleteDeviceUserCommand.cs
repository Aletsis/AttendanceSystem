...
namespace AttendanceSystem.Application.Features.Devices.Commands.DeleteDeviceUser;

public record DeleteDeviceUserCommand(string DeviceId, string UserId) : IRequest<Result>;

public class DeleteDeviceUserHandler : IRequestHandler<DeleteDeviceUserCommand, Result>
{
    private readonly IZKTecoDeviceClient _deviceClient;
    private readonly IDeviceQueries _deviceQueries;

    public DeleteDeviceUserHandler(
    ...
    public async Task<Result> Handle(DeleteDeviceUserCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            try
            {
                var success = await _deviceClient.DeleteUserAsync(request.UserId, cancellationToken);
                return success 
                    ? Result.Success() 
        ...
        {
            return Result.Failure($"Device.DeleteUserException: {ex.Message}");
        }
    }
}
